...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $response = $this->sendPushNotificationToSpecificUsers($translator_array, $job->id, $data, $msg_text, false);       // send new booking push to suitable translators(not delay)
        $response2 = $this->sendPushNotificationToSpecificUsers($delpay_translator_array, $job->id, $data, $msg_text, true); // send new booking push to suitable translators(need to delay)

        return ($response && $response2) ?? false;
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $jobId = $job->id;
        $city = $job->city ?? $jobPosterMeta->city;  // extra variable use removed
        ...
        if ($job->customer_phone_type == 'no') {
            // It's a physical job
            $message = $physicalJobMessageTemplate;
        } else if ($job->customer_phone_type == 'yes') {
            // It's a phone job
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
     ...
     * @return string
     */
    public function updateDistance($job_id, $distance,$time)
    {
        $response = Distance::where('job_id', $job_id)->update(array('distance' => $distance, 'time' => $time));
        return $response;
    }

    public function updateJobs($job_id, $admincomments, $flag, $session, $manually_handled, $by_admin){
        $affectedRows1 = Job::where('id', $job_id)->update(array('admin_comments' => $admincomments, 'flagged' => $flag, 'session_time' => $session,
